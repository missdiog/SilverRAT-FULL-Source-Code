...
public class UploadUserControl1 : UserControl
{
    ...
    private BackgroundWorker bgWorrk;
    ...
    public UploadUserControl1()
    {
        InitializeComponent();
    }

    private void UploadUserControl1_Load(object sender, EventArgs e)
    {
        FileName.Text = TitleFile;
        PathFile.Text = PathFiles;
    }

    public async Task Upload(int index, string UpID)
    {
        if (!File.Exists(UploadFile))
        {
            return;
        }
        SizeFile = new FileInfo(UploadFile).Length;
        Progss.Maximum = Convert.ToInt32(SizeFile);
        await Task.Run(delegate
        {
            Task.Delay(1).Wait();
            try
            {
                using (Icon icon = Icon.ExtractAssociatedIcon(UploadFile))
                {
                    Bitmap image = new Bitmap(icon.ToBitmap(), new Size(30, 30));
                    Progss.Image = image;
                }
                using FileStream fileStream = new FileStream(UploadFile, FileMode.Open, FileAccess.Read, FileShare.Read);
                byte[] array = new byte[1000000];
                MsgPack msgPack = new MsgPack();
                while (fileStream.Read(array, 0, array.Length) > 0 && IsRunning)
                {
                    lock (SyncLock)
                    {
                        msgPack = new MsgPack();
                        ...
                        msgPack.ForcePathObject("UPID").AsString = UpID;
                        msgPack.ForcePathObject("Index").AsString = index.ToString();
                        msgPack.ForcePathObject("FileName").AsString = TitleFile;
                        msgPack.ForcePathObject("SourceFile").AsString = SourceFile;
                        msgPack.ForcePathObject("Size").AsString = SizeFile.ToString();
                        msgPack.ForcePathObject("ByteFile").SetAsBytes(array);
    ...
    private void InitializeComponent()
    {
        ...
        System.ComponentModel.ComponentResourceManager resources = new System.ComponentModel.ComponentResourceManager(typeof(Forms.UploadUserControl1));
        ...
        this.bgWorrk = new System.ComponentModel.BackgroundWorker();
        ...
        this.bunifuShadowPanel_0.Location = new System.Drawing.Point(4, 5);
        ...
        this.bunifuShadowPanel_0.TabIndex = 597;
        ...
        this.WaitingDownload.Size = new System.Drawing.Size(66, 63);
        ...
        this.PathFile.Text = "C:\\Users\\Silver\\Desktop\\New folder (3)\\IconManager";
        ...
        base.Name = "UploadUserControl1";
        base.Size = new System.Drawing.Size(737, 112);
        base.Load += new System.EventHandler(UploadUserControl1_Load);
